...
type Buku struct {
	...
	Harga         int    `json:"harga,omitempty","max:100"`
}
...
func TambahBuku(buku Buku) int64 {
	...
	sqlStatement := `INSERT INTO buku (judul_buku, penulis, tgl_publikasi, harga) VALUES ($1, $2, $3, $4) RETURNING id`
	if buku.Harga > 100 {
		fmt.Println("Harga buku melebihi 100")
	}
	...
	if buku.Harga > 100 {
		log.Fatal("Harga buku jangan melebihi 100")
	}


	err := db.QueryRow(sqlStatement, buku.Judul_buku, buku.Penulis, buku.Tgl_publikasi, buku.Harga).Scan(&id)
...
func AmbilSemuaBuku() ([]Buku, error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(&buku.ID, &buku.Judul_buku, &buku.Penulis, &buku.Tgl_publikasi, &buku.Harga)
...
func UpdateBuku(id int64, buku Buku) int64 {
	...
	// eksekusi sql statement
	res, err := db.Exec(sqlStatement, id, buku.Judul_buku, buku.Penulis, buku.Tgl_publikasi, buku.Harga)
